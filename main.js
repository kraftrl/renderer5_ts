...
function keyPressed(event) {
    ...
    if ('h' == c) {
        ...
        let rot = Matrix.rotateX(15.0);
        let newMatrix = Matrix.build(rot.timesVector(vec1), rot.timesVector(vec2), rot.timesVector(vec3), rot.timesVector(vec4));
        ...
        if (currentPosition < 0)
            currentPosition = scene.positionList.length - 2;
        scene.positionList[currentPosition].model.visible = true;
    }
    const cn = document.getElementById("pixels");
    const ctx = cn.getContext("2d");
